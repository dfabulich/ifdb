...
function parsedownMultilineText($text) {
    ...
    // paragraph breaks.
    // So, we start by replacing all instances of `<br/>` with a replacement token, then run
    // parsedown, which may generate some new `<br />` tokens. We'll replace those with `<br>`
    // (so fixDesc won't turn them into paragraph breaks), then put any legacy `<br/>` tokens
    // back in, so fixDesc can convert those into paragraph breaks.

    // example:
    // in: "1st line \n 2nd line <br /> 3rd line" -> "<p>1st line <br> 2nd line <br /> 3rd line</p>"
    //   fixDesc will convert that to: "1st line <br> 2nd line <p> 3rd line"

    // pre-process iFiction-style <br/> paragraph breaks
    $replacement_token = "IFICBRIFICBR";
    $replaced_ific_br = preg_replace("/<br *\\/>/", $replacement_token, $text);
    $parsedown = new Parsedown();
    $parsedown->setBreaksEnabled(true);
    //$parsedown->setUrlsLinked(false);
    $markdown_converted = $parsedown->text($replaced_ific_br);
    $br_converted = preg_replace("/<br \\/>/", "<br>", $markdown_converted);
    $output = preg_replace("/$replacement_token/", "<br />", $br_converted);
    return $output;
}
